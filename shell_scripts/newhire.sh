...
FILE_SERVER="hostname_of_server_with_logo"
...
# TODO: add check to see if it's in the user's $PATH (?)
GAM_DIR="/Users/$USER/bin/gam/gam"
...
# adds employee to group based on if they are employee or contractor
if [[ ${EMP_TYPE} == [Ee]* ]]; then
    ...
    while [[ ${ANSWER} == y* ]]; do
        ...
        read -p 'member or owner? (m/o) ' priv # while read -p 'member or owner? ' priv && [[ ${PRIV} != [MmOo] ]]; do printf 'Please enter M or O\n' >&2 /dev/null
            if [[ ${PRIV} == [Mm] ]]; 
                then ${GAM_DIR} update group ${GROUP} add member ${USER_VAR}@example.com
            elif [[ ${PRIV} == [Oo] ]]; 
                then ${GAM_DIR} update group ${GROUP} add owner ${USER_VAR}@example.com
            fi
...
# contacts file server to download company logo for email account
...
# could also change this to check for env var but this was a fun little integration
...
JC_USER_ID=$(curl -s -X POST https://console.jumpcloud.com/api/systemusers \
    ...
    -d '{"username":"'${JC_USER_NAME}'", "email":"'${USER_EMAIL}'", "FIRST_NAME":"'${FIRST_NAME}'", \
    "LAST_NAME":"'${LAST_NAME}'", "displayname":"'${FIRST_NAME}' '${LAST_NAME}'", "password":"'${PASSWD}'"}' | jq -r '._id')
